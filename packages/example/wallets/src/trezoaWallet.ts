...
export class TrezoaWallet extends AbstractWallet implements Wallet {
    ...
    constructor() {
        const ledger = {} as TrezoaLedgerApp;
    ...
    #signAndSendTransaction: TrezoaSignAndSendTransactionMethod = async (...inputs) => {
        ...
        for (const { transaction, account, chain, options } of inputs) {
            ...
            if (!account.features.includes('trezoa:signAndSendTransaction')) throw new Error('invalid feature');

            if (!this.chains.includes(chain as TrezoaChain)) throw new Error('invalid chain');
            const endpoint = getEndpointForChain(chain as TrezoaChain);
    ...
    #signTransaction: TrezoaSignTransactionMethod = async (...inputs) => {
        ...
        for (const { transaction, account, chain } of inputs) {
            if (!(account instanceof PossiblyLedgerWalletAccount)) throw new Error('invalid account');
            if (!account.features.includes('trezoa:signTransaction')) throw new Error('invalid feature');

            if (chain && !this.chains.includes(chain as TrezoaChain)) throw new Error('invalid chain');
    ...
    #signMessage: TrezoaSignMessageMethod = async (...inputs) => {
        ...
        for (const { account, message } of inputs) {
            if (!(account instanceof PossiblyLedgerWalletAccount)) throw new Error('invalid account');
            if (!account.features.includes('trezoa:signMessage')) throw new Error('invalid feature');
